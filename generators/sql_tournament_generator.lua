...
sql_tournament_generator.new = function(db_path)
    ...
    function self.record_results(results)
		...
		SQL.writecommand("UPDATE oldcodes SET wins = wins + 1, totalGames = totalGames + 1 WHERE username = \"" .. results.winner.username .. "\"")
        SQL.writecommand("UPDATE oldcodes SET totalGames = totalGames + 1 WHERE username = \"" .. results.loser.username .. "\"")
    end
